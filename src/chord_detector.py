...
def midi_to_note_name(midi_pitch: int) -> str:
    """Return librosa-compatible sharp-spelled notation without lazy imports."""
    names = ("C", "C\u266f", "D", "D\u266f", "E", "F", "F\u266f", "G", "G\u266f", "A", "A\u266f", "B")
    note = int(round(midi_pitch))
...
def score_chord_candidate(pitch_classes: set, bass_pitch: Optional[int],
                         detected_key: str, chord_template: Dict,
                         match_formulation: str = "coverage") -> float:
    ...
    if match_formulation == "coverage":
        denominator = len(template_pcs)
    elif match_formulation == "jaccard":
        denominator = len(pitch_classes | template_pcs)
    else:
        raise ValueError(f"Unsupported chord match formulation: {match_formulation}")

    if denominator == 0:
        match_score = 0.0
    else:
        match_score = len(pitch_classes & template_pcs) / denominator
...
def get_chord_templates() -> List[Dict]:
    ...
    for root in range(12):
        ...
        templates.append({
            ...
            'quality': 'maj',
            'name': f"{midi_to_note_name(root + 60)[:-1]}"
        })
        ...
        templates.append({
            ...
            'quality': 'min',
            'name': f"{midi_to_note_name(root + 60)[:-1]}m"
        })
        ...
        templates.append({
            ...
            'quality': 'sus2',
            'name': f"{midi_to_note_name(root + 60)[:-1]}sus2"
        })
        ...
        templates.append({
            ...
            'quality': 'sus4',
            'name': f"{midi_to_note_name(root + 60)[:-1]}sus4"
        })
        ...
        templates.append({
            ...
            'quality': '7',
            'name': f"{midi_to_note_name(root + 60)[:-1]}7"
        })
        ...
        templates.append({
            ...
            'quality': 'min7',
            'name': f"{midi_to_note_name(root + 60)[:-1]}m7"
        })
        ...
        templates.append({
            ...
            'quality': 'maj7',
            'name': f"{midi_to_note_name(root + 60)[:-1]}maj7"
        })
...
def identify_chord_from_pitches_advanced(
        pitches, bass_pitch=None, detected_key="E major",
        match_formulation="coverage"):
    ...
    for template in templates:
        score = score_chord_candidate(
            pitch_classes, bass_pitch, detected_key, template, match_formulation
        )
...
def identify_chord_for_experiment(pitches, classifier_mode="simple"):
    """Select the frozen classifier variant used by detector experiments."""
    if classifier_mode == "simple":
        return identify_chord_from_pitches(pitches)
    if classifier_mode == "advanced_jaccard":
        bass_pitch = min(pitches) if pitches else None
        return identify_chord_from_pitches_advanced(
            pitches,
            bass_pitch=bass_pitch,
            detected_key="E major",
            match_formulation="jaccard",
        )
    raise ValueError(f"Unsupported chord classifier mode: {classifier_mode}")
...
def analyze_midi_chords(midi_path: str, window_size: Optional[float] = None,
                        use_beat_sync: bool = False, audio_path: Optional[str] = None,
                        classifier_mode: str = "simple",
                        runtime_trace: Optional[Dict] = None) -> Dict:
    """Main function to analyze MIDI and extract chord progression with professional musical intelligence"""
    harmony_started = perf_counter()
    harmony_timings = runtime_trace["stages_sec"] if runtime_trace is not None else None
    
    # Parse MIDI file first to get data for auto-detection
    parse_started = perf_counter()
    notes, total_duration = parse_midi_notes(midi_path)
    if harmony_timings is not None:
        harmony_timings["midi_parsing"] = perf_counter() - parse_started
    if not notes:
        return None
    ...
    if use_beat_sync and audio_path and use_professional_ai:
        ...
        if harmony_timings is not None:
            harmony_timings["window_construction"] = perf_counter() - window_started
        ...
        for i, window in enumerate(windows):
            ...
            chord_name = identify_chord_for_experiment(unique_pitches, classifier_mode)
            if harmony_timings is not None:
                harmony_timings["candidate_scoring"] = (
                    harmony_timings.get("candidate_scoring", 0.0)
    ...
    print(f"Total chords: {len(grouped_chords)}")
    ...
    if harmony_timings is not None:
        harmony_timings["harmony_postprocessing"] = perf_counter() - postprocess_started
        harmony_timings["harmony_total"] = perf_counter() - harmony_started
    return results
