...
try:
    ...
    chord_tutor = ChordAIRAGTutor()
    print("✅ ChordAI GPT Tutor instance initialized")
except ImportError as e:
    print(f"⚠️ ChordAI GPT Tutor not available: {e}")
    ChordAIRAGTutor = None
    chord_tutor = None
except Exception as e:
    print(f"⚠️ ChordAI GPT Tutor initialization failed: {e}")
    ChordAIRAGTutor = None
    chord_tutor = None
...
class TutorWebSocketManager:
    ...
    async def analyze_audio_streaming(self, client_id: str, audio_file_path: str, user_question: Optional[str] = None):
        """Analyze audio and stream tutoring response with TTS."""
        try:
            if not self.tutor:
            ...
            await self.send_message(client_id, {
                "type": "status",
                "message": "Starting chord analysis..."
            })
            
            # Step 1: Run chord detection
            print(f"🔍 Analyzing audio: {audio_file_path}")
            chord_results = self.tutor._run_horizon_jam(audio_file_path)
            
            # Send chord analysis results
            await self.send_message(client_id, {
                "type": "chord_analysis",
                "data": chord_results
            })
            ...
            rag_context = self.tutor._retrieve_rag_context(chord_results, user_question)
            
            # Send RAG context to client for display
            await self.send_message(client_id, {
                "type": "rag_context",
                "data": {
                    "contexts": [doc.page_content for doc in rag_context.get('contexts', [])],
                    "query": rag_context.get('query', ''),
                    "total_docs": len(rag_context.get('contexts', []))
                }
            })
            
            # Step 3: Start streaming tutoring with TTS
            await self.send_message(client_id, {
                "type": "status",
            ...
            full_response = self.tutor.stream_rag_tutoring(
                ...
                websocket_callback=lambda text: asyncio.create_task(tts_callback(text)),
...
<body>
    ...
    <script>
        ...
        function displayChordAnalysis(analysis) {
            ...
            const accuracy = summary.estimated_accuracy_percent || summary.estimated_accuracy || (chordEvents.length > 0 ? '85.0' : 'Unknown');
...
async def upload_audio(audio_file: UploadFile = File(...), question: Optional[str] = Form(None)):
    ...
    try:
        ...
        file_path = temp_dir / f"upload_{audio_file.filename}"
